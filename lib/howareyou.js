(function ($) {
  ...
  current_config = user || empty_user;
  ...
  $(function () {
    ...
    $('#howareyou_login_demo').click(function (event) {
      event.preventDefault();
      $('#howareyou_login_email').val('demo@howareyou.com');
      $('#howareyou_login_password').val('Password1');
      $('#howareyou_login_modal form').submit();
    });
    ...
    $('#howareyou_logout').click(function (event) {
      ...
      set_config(empty_user);
    });

    $('#howareyou_use_default_keys').click(function (event) {
      event.preventDefault();
      set_credestials_ui(empty_user);
    });
  ...
  function forget () {
    if (LS) {
      LS.removeItem(LOCAL_STORAGE_USER_KEY);
    }
  }
  ...
  function remind () {
    var value;
    if (LS) {
      value = LS.getItem(LOCAL_STORAGE_USER_KEY);
      if (value) {
